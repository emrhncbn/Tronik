...
*/
// Mouse tuşu veya bilgisayar tuşlarına basıldığında synth tuşlarını işle
...
const handleKeyDown = (event) => {
  ...
  const targetKey = synthKeys.find((key) => key.computerKey === pressedKey)
  if (targetKey) {
    setSynthKeys((prevKeys) =>
      prevKeys.map((key) =>
        key === targetKey ? { ...key, active: true, keyPressed: true } : key
      )
    )
  }
}
...
const handleKeyUp = (event) => {
  ...
  const targetKey = synthKeys.find((key) => key.computerKey === releasedKey)
  if (targetKey) {
    setSynthKeys((prevKeys) =>
      prevKeys.map((key) =>
        key === targetKey ? { ...key, active: false, keyPressed: false } : key
      )
    )
  }
}

// Global klavye olaylarını dinlemek için event listenerlar ekle
useEffect(() => {
